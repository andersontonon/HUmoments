...
def main():
    ...
    trainFeatures = HuMoments(trainImages)
    ...
    testFeatures = HuMoments(testImages)
...
def HuMoments(images):
    bar = Bar('[INFO] Extracting shape...',max=len(images),suffix='%(index)d/%(max)d  Duration:%(elapsed)ds')
    huMomentsList = []
    for img in images:
        if (np.ndim(img) > 2): # > 2 = colorida
            gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            ret,thresh = cv2.threshold(gray,170,255,0)
            contours,hierarchy = cv2.findContours(thresh, cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)
            print("Number of contours detected:",len(contours))
            huMoments  = cv2.HuMoments(cv2.moments(thresh))
            huMomentsList.append(huMoments.flatten())
            bar.next()
        bar.finish()
    return np.array(huMomentsList)
